...
- Build new UI work in `public-v2`. Treat `public` as the old UI unless explicitly asked otherwise.
- The v2 aesthetic is mobile-first, dark green, thumb-friendly, and minimal.
- Use the existing v2 patterns: phone-shaped desktop frame, bottom icon tab tray, swipe action trays, sheets, compact cards, chat bubbles, and restrained touch animations.
...
- Admin should use the same v2 aesthetic. Avoid desktop-first tables when compact cards or tabbed views will do.
- When changing JS/CSS in `public-v2`, bump the query string in `public-v2/index.html` so the PWA refetches assets.
- If service-worker behavior changes, bump `self.WOOD_SW_VERSION` in `public-v2/sw.js`.
...
- Use `.env` for local settings. Common local v2 settings are:
...
- For frontend-only v2 tweaks, at least run `node --check public-v2/app.js`. Run the full test suite when practical.
...
- Good examples:
  - `Fix v2 history scrolling`
  - `Add fake Wood keyboard composer`
  - `Anchor v2 bottom tabs in desktop frame`

## Current Product Bets

- `public-v2` is the path forward.
